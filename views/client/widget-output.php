...
foreach ( $stats as $stat ) {
    if ($use_default == 1) {
        //default output
        $output .= '<span class="taw-tutorials"><a title="' . esc_attr__('See all tutorials', TAW_TEXTDOMAIN) . '" href="' . esc_url(sprintf('https://fr.tuto.com/formateur/%s.htm', $apilogin)) . '">' . __('tutorials : ', TAW_TEXTDOMAIN) . $stat['tutorials'] . '</a></span>';
        $output .= '<span class="taw-customers">' . __('customers : ', TAW_TEXTDOMAIN) . $stat['customers'] . '</span>';
